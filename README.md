...
- 메인 브랜치: `main`(배포용),  `develop`(테스트용 메인)
- 작업 브랜치: 기능 개발 시 `develop` 브랜치에서 `타입/#이슈번호-작업내용` 형식의 브랜치명으로 새로운 브랜치를 파서 작업
  (ex: feature/#1-mypage, fix/#10-UIFix 등)
- 커밋 규칙: 깃허브에 Push한 뒤, `develop` 브랜치로 PR 생성 (*Notion의 컨벤션 준수해 주세요.*)
- 보호 규칙: `main`과 `develop` 브랜치에 직접 Merge 불가
